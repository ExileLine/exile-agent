...
- 支持服务端 Approval Store，审批场景优先通过 `approval_id + approvals` 续跑，避免客户端回传完整 message history
...
`message_history_json` 暂时仍保留为兼容旧客户端的 fallback 字段，新客户端应优先使用 `approval_id`。
...
旧版 `message_history_json + approvals` 仍可临时兼容。
...
- `AgentRunner._build_chat_response(...)` 识别到这一点后，返回：
  ...
  - `deferred_tool_requests.approval_id`
  - `deferred_tool_requests.expires_at`
...
- `resume_chat(...)` 做两件事：
  - 优先用 `approval_id` 从服务端 `ApprovalStore` 读取上一轮运行上下文
...
- `/chat` 的核心字段是 `message`
- `/chat/resume` 的核心字段是 `approval_id + approvals`
- 旧版客户端仍可临时使用 `message_history_json + approvals`
...
兼容旧客户端时，也可以继续传 `message_history_json + approvals`，但新协议应优先使用 `approval_id`，由服务端读取已保存的 message history。
...
- 输出内容：
  ...
  - `calls`
  - `message_history_json`
- 随后 `_attach_approval_record(...)` 会创建服务端审批单并补充：
  - `approval_id`
  - `expires_at`
  - `status`
...
- 方法：`resume_chat(...)`
- 关键动作：
  - 通过 `approval_id` 读取 `ApprovalStore`
  - 校验审批状态、过期时间、用户和 `tool_call_id`
...
- 当前已有服务端 Approval Store 基线
  - 支持 `approval_id` 续跑、过期校验、重复续跑拒绝
  - 还缺审批单查询/撤销、args hash 校验和生命周期审计
- 当前审批判断仍是最小规则
  - 只看 tool metadata
